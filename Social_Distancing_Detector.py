...
while cap.isOpened():
    ...
    ret, frame = cap.read() # ndarray with shape (250, 444, 3)

    if not ret: # not True
        break

    (h, w) = frame.shape[:2] # 250 , 444

    # Resize the frame to suite the model requirements. Resize the frame to 300X300 pixels
    blob = cv2.dnn.blobFromImage(cv2.resize(frame, (300, 300)), 0.007843, (300, 300), 127.5)

    network.setInput(blob)
    detections = network.forward() #(1, 1, 100, 7)
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            # Filtering only persons detected in the frame. Class Id of 'person' is 15 in labels
            if class_id == 15.00:
                ...
                distance = (165 * F)/height # assumed that the average height of humans in 165 centimetres
